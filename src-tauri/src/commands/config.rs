...
pub async fn install_qqbot_plugin() -> Result<String, String> {
    ...
    // 安装 QQ Bot 插件（QQ 开放平台文档：@tencent-connect/openclaw-qqbot）
    info!("[QQ Bot 插件] 执行 openclaw plugins install @tencent-connect/openclaw-qqbot@latest ...");
    match shell::run_openclaw(&[
        "plugins",
        "install",
        ...
        Err(e) => {
            ...
            Err(format!(
                "安装 QQ Bot 插件失败: {}\n\n请手动执行: openclaw plugins install @tencent-connect/openclaw-qqbot@latest",
                e
            ))
        }
    }
