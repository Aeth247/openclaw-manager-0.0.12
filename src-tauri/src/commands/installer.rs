...
pub async fn check_environment() -> Result<EnvironmentStatus, String> {
    ...
    // 检查 OpenClaw：与 get_system_info 一致——以能否找到 CLI 路径为「已安装」；
    // 「就绪」仍要求能成功执行 --version（见下方 ready）
    info!("[环境检查] 检查 OpenClaw...");
    let openclaw_path_ok = shell::get_openclaw_path().is_some();
    let openclaw_version = if openclaw_path_ok {
        get_openclaw_version()
    } else {
        None
    };
    let openclaw_installed = openclaw_path_ok;
    info!(
        "[环境检查] OpenClaw: path_ok={}, installed_flag={}, version={:?}",
        openclaw_path_ok, openclaw_installed, openclaw_version
    );
